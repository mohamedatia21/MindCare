...
/**
 * Google Gemini LLM Provider adapter using Gemini's OpenAI-compatible endpoint.
 * Conforms strictly to the existing StreamingLLMProvider and LLMProvider contracts.
 */
export class GeminiStreamingLLM implements StreamingLLMProvider, LLMProvider {
  ...
  public async *generateStreamingResponse(request: LLMRequest): AsyncGenerator<string, StructuredLLMOutput, void> {
    if (!this.client) {
      this.logger.warn('Gemini missing credentials. Aborting.', { requestId: 'gemini', timestamp: new Date() });
    ...
    try {
      ...
      this.logger.error('GeminiStreamingError', { requestId: 'gemini', error: err, timestamp: new Date() });
